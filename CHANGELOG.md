...
* **Test a Request.** A read-only preview tool under Settings > Site Protection: paste a URL, IP, or User-Agent and see exactly what Shield would do with it -- blocked, flagged, or allowed, and why -- before you turn any blocking switch on. Runs through the exact same checks the live gate uses; never writes to the attack log or affects real traffic. Scoped to this edition's actual feature set (no Referrer/Spamhaus preview, since those checks aren't part of the Free edition).
